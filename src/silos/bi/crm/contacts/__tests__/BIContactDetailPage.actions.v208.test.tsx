...
describe("BF_PORTAL_BLOCK_v208 — Contact actions", () => {
  ...
  it("clicking Call starts the call with the contact phone", async () => {
    apiMock.mockImplementation(makeApi());
    renderAtId("c1");
    await waitFor(() => screen.getByText("Jane Doe"));
    fireEvent.click(screen.getByTestId("bi-contact-call-button"));
    await waitFor(() => expect(openDialerMock).toHaveBeenCalled());
    expect(openDialerMock).toHaveBeenCalledWith("+14165551234", {
      contactId: "c1",
...
describe("BF_PORTAL_BLOCK_v208 — Send SMS", () => {
  ...
  it("composer toggles open/closed", async () => {
    ...
    renderAtId("c1");
    await waitFor(() => screen.getByText("Jane Doe"));
    fireEvent.click(screen.getByTestId("bi-contact-sms-button"));
    expect(screen.getByTestId("bi-contact-sms-composer")).toBeInTheDocument();
    fireEvent.click(screen.getByTestId("bi-contact-sms-button"));
    expect(screen.queryByTestId("bi-contact-sms-composer")).toBeNull();
  });

  it("disabled when contact has no phone", async () => {
    apiMock.mockImplementation(
      makeApi({ contact: { ...baseContact, phone_e164: null } }),
    );
    renderAtId("c1");
    await waitFor(() => screen.getByText("Jane Doe"));
    const btn = screen.getByTestId("bi-contact-sms-button");
    expect(btn).toBeDisabled();
  });

  it("POST /sms is called with the body and refreshes", async () => {
    apiMock.mockImplementation(makeApi());
    renderAtId("c1");
    await waitFor(() => screen.getByText("Jane Doe"));
    fireEvent.click(screen.getByTestId("bi-contact-sms-button"));
    fireEvent.change(screen.getByLabelText(/^sms body$/i), {
      target: { value: "Reminder: demo at 2pm" },
    });
  ...
  it("surfaces server error", async () => {
    apiMock.mockImplementation(
      makeApi({ throws: { POST: new Error("twilio_21408") } }),
    );
    renderAtId("c1");
    await waitFor(() => screen.getByText("Jane Doe"));
    fireEvent.click(screen.getByTestId("bi-contact-sms-button"));
    ...
    fireEvent.click(screen.getByRole("button", { name: /^send$/i }));
    await waitFor(() => {
      expect(screen.getByText(/twilio_21408/)).toBeInTheDocument();
    });
  });
});
