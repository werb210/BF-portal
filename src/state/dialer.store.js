...
const isCallInProgressStatus = (status) => typeof status === "string" && CALL_IN_PROGRESS_STATUSES.includes(status);
const createCallId = () => {
...
const resolveOutcome = (status, outcome) => {
    if (outcome)
        return outcome;
    if (status === "connected")
        return "completed";
...
const resolveFinalStatus = (status, outcome) => {
    if (outcome === "completed")
        return "completed";
    if (outcome === "voicemail")
        return "voicemail";
    if (outcome === "no-answer" || outcome === "failed" || outcome === "canceled")
        return "failed";
    return status === "connected" ? "completed" : "failed";
};
const resolveFailureReason = (status, outcome, failureReason) => {
    if (failureReason)
...
const restoredElapsed = hasActiveSessionCall && sessionStartedAt ? sessionElapsed : (sessionState.elapsedSeconds ?? 0);
const storedLogs = readLogs();
const { logs: hydratedLogs, pendingFlushLogs } = (() => {
    if (hasActiveSessionCall) {
        return { logs: storedLogs, pendingFlushLogs: [] };
    }
    const flushable = [];
...
export const useDialerStore = create((set, get) => ({
    ...
    setStatus: (status) => set((state) => {
        if (!state.currentCallId)
            return { status };
        const logs = state.logs.map((log) => log.callId === state.currentCallId ? { ...log, status } : log);
        return { status, logs };
    }),
    recordElapsed: (seconds) => set({ elapsedSeconds: seconds }),
    registerDialAttempt: (number) => set((state) => {
