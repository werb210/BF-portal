...
const normalizeString = (value) => value.trim().toUpperCase();
...
export const compareOcrResults = (results) => {
    const filteredResults = results.filter((result) => OCR_FIELD_REGISTRY_MAP.has(result.field_key));
    const resultsByField = filteredResults.reduce((acc, result) => {
        if (!acc[result.field_key])
            acc[result.field_key] = [];
    ...
    Object.entries(resultsByField).forEach(([fieldKey, fieldResults]) => {
        ...
        const hasMismatch = uniqueValues.some((value, index) => uniqueValues.slice(index + 1).some((other) => !valuesMatch(field, value, other)));
        if (!hasMismatch)
            return;
