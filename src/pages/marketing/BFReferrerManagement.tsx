...
// BF staff view of the BF referrer system (Marketing > Referrers). Mirrors the
// BI Referrer Management view but wired to BF data via /api/admin/referrers.
...
export default function BFReferrerManagement() {
  ...
  async function load() {
    ...
    try {
      const r: any = await api<any>("/api/admin/referrers");
      const list: Referrer[] = Array.isArray(r) ? r : (Array.isArray(r?.referrers) ? r.referrers : []);
      setReferrers(list);
    } catch (e) {
      // Endpoint provided by BF-Server BLOCK v1_ADMIN_REFERRERS.
  ...
  async function openDetail(id: string) {
    ...
    try {
      const d: any = await api<any>(`/api/admin/referrers/${id}/detail`);
