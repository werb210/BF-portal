...
import { createLenderProduct, fetchLenderProducts, fetchLenders, updateLenderProduct } from "@/api/lenders";
import { LENDER_PRODUCT_CATEGORIES, LENDER_PRODUCT_CATEGORY_LABELS } from "@/types/lenderManagement.types";
import { PRODUCT_DOCUMENT_OPTIONS, buildRequiredDocumentsPayload, deriveCurrency, deriveProductName, formatInterestPayload, getDefaultRequiredDocuments, isValidVariableRate, mapRequiredDocumentsToValues, normalizeCountrySelection, splitCountrySelection, normalizeInterestInput, resolveRateType } from "./lenderProductForm";
...
const RATE_TYPE_OPTIONS = ["fixed", "variable"];
...
const LenderProductsContent = () => {
    ...
    const [activeLenderId, setActiveLenderId] = useState("");
    ...
    useEffect(() => {
        if (!activeLenderId)
            return;
        if (activeLenders.some((lender) => lender.id === activeLenderId))
            return;
        setActiveLenderId(activeLenders[0]?.id ?? "");
    }, [activeLenderId, activeLenders]);
    const activeLender = useMemo(() => activeLenders.find((lender) => lender.id === activeLenderId) ?? null, [activeLenderId, activeLenders]);
    ...
    const visibleProducts = useMemo(() => {
        ...
        return safeProducts.filter((product) => activeIds.has(product.lenderId) && (!activeLenderId || product.lenderId === activeLenderId));
    }, [activeLenderId, activeLenders, safeProducts]);
    const hasStartupCategory = useMemo(() => visibleProducts.some((product) => product.category === "STARTUP_CAPITAL") || editingProduct?.category === "STARTUP_CAPITAL", [visibleProducts, editingProduct]);
    const selectedProduct = useMemo(() => visibleProducts.find((product) => product.id === selectedProductId) ?? null, [visibleProducts, selectedProductId]);
    useEffect(() => {
        if (isNewRoute) {
    ...
    const buildPayload = (values, existing) => {
    ...
    if (lendersLoading || productsLoading)
        return _jsx(AppLoading, {});
    if (lendersError || productsError) {
        return (_jsx(ErrorBanner, { message: getErrorMessage(lendersError ?? productsError, "Unable to load lender products."), onRetry: () => {
                refetchLenders();
                refetchProducts();
            } }));
    }
    return (_jsxs("div", { className: "page page--lender-products", children: [_jsxs("div", { className: "page-header", children: [_jsxs("div", { children: [_jsx("h1", { children: "Lender products" }), _jsx("p", { className: "page-header__subtitle", children: "Manage lender products and submission requirements." })] }), _jsx(ActionGate, { actions: ["create_lender_product"], children: _jsx(Button, { variant: "primary", onClick: () => {
                                setEditingProduct(null);
                                setSelectedProductId(null);
                                setFormValues(emptyProductForm(activeLenderId));
                                setFormErrors({});
                                setSubmitError(null);
                                setIsModalOpen(true);
                            }, disabled: !activeLenderId, children: "Add product" }) })] }), _jsx(Card, { children: _jsxs("div", { className: "card__body space-y-4", children: [_jsxs("div", { className: "management-grid", children: [_jsx(Select, { label: "Active lender", value: activeLenderId, onChange: (event) => setActiveLenderId(event.target.value), children: activeLenders.map((lender) => (_jsx("option", { value: lender.id, children: lender.name || "Unnamed lender" }, lender.id))) }), !activeLenders.length && (_jsx("div", { className: "text-sm text-amber-600", children: "No active lenders available." }))] }), !activeLenders.length ? null : (_jsx("div", { className: "space-y-4", children: CATEGORY_DISPLAY_ORDER.map((category) => {
                                const productsInCategory = visibleProducts.filter((product) => product.category === category);
                                if (!productsInCategory.length)
                                    return null;
                                return (_jsxs("div", { className: "space-y-2", children: [_jsx("h3", { children: PORTAL_PRODUCT_CATEGORY_LABELS[category] ?? LENDER_PRODUCT_CATEGORY_LABELS[category] }), _jsx(Table, { headers: [
                                                "Product name",
                                                "Lender",
                                                "Submission",
                                                "Active",
                                                ...
                                                return (_jsxs("tr", { children: [_jsx("td", { children: product.productName }), _jsx("td", { children: lender?.name ?? "—" }), _jsx("td", { children: renderSubmissionMethodBadge(lenderMethod) }), _jsx("td", { children: _jsx("span", { className: `status-pill status-pill--${isProductActive ? "active" : "paused"}`, children: isProductActive ? "Active" : "Inactive" }) }), _jsxs("td", { children: ["$", product.minAmount.toLocaleString(), " - $", product.maxAmount.toLocaleString()] }), _jsx("td", { children: formatLenderCountryLabel(product.country) }), _jsx("td", { children: formatRateType(product.rateType) }), _jsxs("td", { children: [_jsx(Button, { variant: "ghost", size: "sm", onClick: () => {
                                                                        navigate(`/lender-products/${product.id}`);
                                                                    }, children: "View" }), _jsx(Button, { variant: "ghost", size: "sm", onClick: () => {
                                                                        setEditingProduct(product);
                                                                        setSelectedProductId(product.id);
                                                                        setIsModalOpen(true);
                                                                    }, children: "Edit" })] })] }, product.id));
                                            }) })] }, category));
                            }) }))] }) }), _jsx(LenderProductModal, { isOpen: isModalOpen, title: editingProduct ? "Edit product" : "Add product", isSaving: mutationLoading, errorMessage: submitError, lenderOptions: activeLenders.map((lender) => ({ value: lender.id, label: lender.name || "Unnamed lender" })), formValues: formValues, formErrors: formErrors, categoryOptions: CATEGORY_DISPLAY_ORDER.map((category) => ({
                    value: category,
                    label: PORTAL_PRODUCT_CATEGORY_LABELS[category] ?? LENDER_PRODUCT_CATEGORY_LABELS[category]
                })), rateTypes: RATE_TYPE_OPTIONS, documentOptions: PRODUCT_DOCUMENT_OPTIONS, formatRateType: formatRateType, onChange: updateForm, onSubmit: handleSubmit, onClose: closeModal, onCancel: closeModal, statusNote: submitError ? (_jsx(Button, { type: "button", variant: "secondary", onClick: handleSubmit, disabled: mutationLoading, children: "Retry save" })) : null })] }));
};
const LenderProductsPage = () => (_jsx(RequireRole, { roles: ["Admin", "Staff"], children: _jsx(LenderProductsContent, {}) }));
