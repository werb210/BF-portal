...
const parseStage = (item) => {
    ...
    const allowedTransitions = Array.isArray(record.allowedTransitions)
        ? record.allowedTransitions.filter((stageId) => typeof stageId === "string")
        : Array.isArray(record.nextStages)
            ? record.nextStages.filter((stageId) => typeof stageId === "string")
            : undefined;
    return {
        id,
...
const parseStringArray = (value) => Array.isArray(value) ? value.filter((item) => typeof item === "string") : undefined;
...
const resolveStage = (stage, status) => {
...
const normalizePipelineApplication = (value) => {
    if (!isRecord(value))
        return null;
    ...
    const documents = isRecord(value.documents)
        ? {
            submitted: typeof value.documents.submitted === "number" ? value.documents.submitted : undefined,
            required: typeof value.documents.required === "number" ? value.documents.required : undefined
        }
        : {
            submitted: typeof value.documents_submitted === "number" ? value.documents_submitted : undefined,
            required: typeof value.documents_required === "number" ? value.documents_required : undefined
        };
    return {
        id,
        businessName: typeof value.businessName === "string"
            ...
            : typeof value.business_name === "string"
                ? value.business_name
                : undefined,
        contactName: typeof value.contactName === "string"
            ? value.contactName
            : typeof value.contact_name === "string"
                ? value.contact_name
                : undefined,
        requestedAmount: typeof value.requestedAmount === "number"
            ? value.requestedAmount
            : typeof value.requested_amount === "number"
                ? value.requested_amount
                : undefined,
        productCategory: typeof value.productCategory === "string"
            ? value.productCategory
            : typeof value.product_category === "string"
                ? value.product_category
                : undefined,
        submissionMethod: typeof value.submissionMethod === "string"
        ...
        matchPercentage: typeof value.matchPercentage === "number" || typeof value.matchPercentage === "string"
            ? value.matchPercentage
            : typeof value.match_percentage === "number" || typeof value.match_percentage === "string"
                ? value.match_percentage
                : undefined,
        matchPercent: typeof value.matchPercent === "number" || typeof value.matchPercent === "string"
            ? value.matchPercent
            : typeof value.match_percent === "number" || typeof value.match_percent === "string"
                ? value.match_percent
                : undefined,
        matchScore: typeof value.matchScore === "number" || typeof value.matchScore === "string"
            ? value.matchScore
            : typeof value.match_score === "number" || typeof value.match_score === "string"
                ? value.match_score
                : undefined,
        documents,
        bankingComplete: typeof value.bankingComplete === "boolean"
            ? value.bankingComplete
            : typeof value.banking_complete === "boolean"
                ? value.banking_complete
                : undefined,
        ocrComplete: typeof value.ocrComplete === "boolean"
            ? value.ocrComplete
            : typeof value.ocr_complete === "boolean"
                ? value.ocr_complete
                : undefined,
        ocrMissingFields: parseStringArray(value.ocrMissingFields) ??
            parseStringArray(value.ocr_missing_fields) ??
            parseStringArray(value.missing_ocr_fields),
        ocrConflictCount: typeof value.ocrConflictCount === "number"
        ...
        referrerId: typeof value.referrerId === "string"
            ? value.referrerId
            : typeof value.referrer_id === "string"
                ? value.referrer_id
                : undefined,
        referrerName: typeof value.referrerName === "string"
            ? value.referrerName
            : typeof value.referrer_name === "string"
                ? value.referrer_name
                : undefined,
        assignedStaff: typeof value.assignedStaff === "string"
            ? value.assignedStaff
            : typeof value.assigned_staff === "string"
                ? value.assigned_staff
                : undefined,
        assignedLender: typeof value.assignedLender === "string"
...
const parsePipelineApplications = (data) => {
...
const parsePipelineResponse = (data) => {
    if (Array.isArray(data)) {
        return { stages: [], applications: parsePipelineApplications(data) };
    }
    if (!isRecord(data)) {
    ...
    const rawApplications = Array.isArray(data.applications)
        ...
        : Array.isArray(data.items)
            ...
            : Array.isArray(data.data)
                ...
                : [];
    return {
        stages: rawStages.map(parseStage).filter((stage) => Boolean(stage)),
        applications: parsePipelineApplications(rawApplications)
    };
};
const buildLockedStages = () => PIPELINE_STAGE_ORDER.map((stageId) => ({
    id: stageId,
    label: PIPELINE_STAGE_LABELS[normalizeStageId(stageId)] ?? toTitleCase(stageId)
}));
const buildPipelineQuery = (filters) => {
    if (!filters)
        return "";
    ...
    const businessUnit = filters.businessUnit;
    if (businessUnit)
...
export const pipelineApi = {
    fetchPipeline: async (filters, options) => {
        const res = await api.get(`${API_PREFIX}/pipeline${buildPipelineQuery(filters)}`, options);
    ...
    fetchStages: async (options) => {
        const { stages } = await pipelineApi.fetchPipeline(undefined, options);
        return stages;
    },
    fetchColumn: async (stage, filters, options) => {
    ...
    exportApplications: async (applicationIds) => {
